...
static inline SysRes VG_UCONTEXT_SYSCALL_SYSRES(void *ucV, UWord scclass) {
...
static inline void VG_UCONTEXT_TO_UnwindStartRegs(UnwindStartRegs *srP,
                                                  void *ucV) {
  ucontext_t *uc = (ucontext_t *)(ucV);
  struct __darwin_mcontext32 *mc = uc->uc_mcontext;
  struct __darwin_i386_thread_state *ss = &mc->__ss;
...
#define VG_UCONTEXT_SYSCALL_SYSRES(uc)                                         \
  ...
  ((uc)->uc_mcontext.sc_regs[2], (uc)->uc_mcontext.sc_regs[3],                 \
...
#define VG_UCONTEXT_SYSCALL_SYSRES(uc)                                         \
  ...
  ((uc)->uc_mcontext.gregs[VKI_EFL] & 1, (uc)->uc_mcontext.gregs[VKI_EAX],     \
   (uc)->uc_mcontext.gregs[VKI_EFL] & 1 ? 0                                    \
                                        : (uc)->uc_mcontext.gregs[VKI_EDX])
#define VG_UCONTEXT_TO_UnwindStartRegs(srP, uc)                                \
  {                                                                            \
...
#define VG_UCONTEXT_SYSCALL_SYSRES(uc)                                         \
  ...
  ((uc)->uc_mcontext.gregs[VKI_REG_RFL] & 1,                                   \
   ...
   (uc)->uc_mcontext.gregs[VKI_REG_RFL] & 1                                    \
       ? 0                                                                     \
       : (uc)->uc_mcontext.gregs[VKI_REG_RDX])
#define VG_UCONTEXT_TO_UnwindStartRegs(srP, uc)                                \
  {                                                                            \
...
/* Let's do:
   ...
   Flags:
     ...
     SA_NOMASK or SA_NODEFER -- we observe this, but our handlers block
   everything SA_ONSTACK -- pass through SA_NOCLDWAIT -- pass through
*/
...
static void pp_SKSS(void) {
  Int sig;
  VG_(printf)("\n\nSKSS:\n");
  for (sig = 1; sig <= _VKI_NSIG; sig++) {
    VG_(printf)
    ("sig %d:  handler %p,  flags 0x%x\n", sig,
     skss.skss_per_sig[sig].skss_handler, skss.skss_per_sig[sig].skss_flags);
  }
}
...
/* This is the core, clever bit.  Computation is as follows:
   ...
   For each signal
      handler = if client has a handler, then our handler
                        ...
                        then hander is IGN
*/
static void calculate_SKSS_from_SCSS(SKSS *dst) {
  Int sig;
  UInt scss_flags;
  UInt skss_flags;
  ...
  for (sig = 1; sig <= VG_(max_signal); sig++) {
...
#if !defined(VGP_ppc32_linux) && !defined(VGP_x86_darwin) &&                   \
    !defined(VGP_amd64_darwin) && !defined(VGP_mips32_linux) &&                \
    !defined(VGO_solaris)
    ...
    */
    ...
    ("setting ksig %d to: hdlr %p, flags 0x%lx, "
     ...
     sig, ksa.ksa_handler, (UWord)ksa.sa_flags,
    ...
    if (!force_update) {
      vg_assert(ksa_old.ksa_handler == skss_old.skss_per_sig[sig].skss_handler);
#if defined(VGO_solaris)
...
#endif
      vg_assert(ksa_old.sa_flags == skss_old.skss_per_sig[sig].skss_flags);
#if !defined(VGP_ppc32_linux) && !defined(VGP_x86_darwin) &&                   \
    !defined(VGP_amd64_darwin) && !defined(VGP_mips32_linux) &&                \
...
SysRes VG_(do_sys_sigaltstack)(ThreadId tid, vki_stack_t *ss,
                               vki_stack_t *oss) {
  Addr m_SP;
  ...
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("sys_sigaltstack: tid %u, "
   "ss %p{%p,sz=%llu,flags=0x%llx}, oss %p (current SP %p)\n",
   tid, (void *)ss, ss ? ss->ss_sp : 0, (ULong)(ss ? ss->ss_size : 0),
   (ULong)(ss ? ss->ss_flags : 0), (void *)oss, (void *)m_SP);

  if (oss != NULL) {
    oss->ss_sp = VG_(threads)[tid].altstack.ss_sp;
    oss->ss_size = VG_(threads)[tid].altstack.ss_size;
    oss->ss_flags =
        VG_(threads)[tid].altstack.ss_flags | sas_ss_flags(tid, m_SP);
  }
  ...
  if (ss != NULL) {
    ...
    if (ss->ss_flags != VKI_SS_DISABLE && ss->ss_flags != VKI_SS_ONSTACK &&
        ss->ss_flags != 0) {
...
SysRes VG_(do_sys_sigaction)(Int signo, const vki_sigaction_toK_t *new_act,
  ...
  if (signo < 1 || signo > VG_(max_signal))
    goto bad_signo;

  /* don't let them use our signals */
  if ((signo > VG_SIGVGRTUSERMAX) && new_act &&
...
static void do_sigprocmask_bitops(Int vki_how, vki_sigset_t *orig_set,
...
static HChar *format_sigset(const vki_sigset_t *set) {
...
#if _VKI_NSIG_BPW == 32
    VG_(sprintf)
    (buf + VG_(strlen)(buf), "%08llx", set ? (ULong)set->sig[w] : 0);
#elif _VKI_NSIG_BPW == 64
    VG_(sprintf)
    (buf + VG_(strlen)(buf), "%16llx", set ? (ULong)set->sig[w] : 0);
#else
...
*/
static void do_setmask(ThreadId tid, Int how, vki_sigset_t *newset,
                       vki_sigset_t *oldset) {
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("do_setmask: tid = %u how = %d (%s), newset = %p (%s)\n", tid, how,
   how == VKI_SIG_BLOCK
       ? "SIG_BLOCK"
       : (how == VKI_SIG_UNBLOCK
              ? "SIG_UNBLOCK"
...
SysRes VG_(do_sys_sigprocmask)(ThreadId tid, Int how, vki_sigset_t *set,
                               vki_sigset_t *oldset) {
...
static void block_all_host_signals(/* OUT */ vki_sigset_t *saved_mask) {
  ...
  ret = VG_(sigprocmask)(VKI_SIG_SETMASK, &block_procmask, saved_mask);
  vg_assert(ret == 0);
}
...
/* Set up a stack frame (VgSigContext) for the client's signal
   handler. */
static void push_signal_frame(ThreadId tid, const vki_siginfo_t *siginfo,
  ...
  if (/* this signal asked to run on an alt stack */
      (scss.scss_per_sig[sigNo].scss_flags &
       VKI_SA_ONSTACK) && /* there is a defined and enabled alt stack, which
                             we're not already using.  Logic from get_sigframe
                             in arch/i386/kernel/signal.c. */
      sas_ss_flags(tid, VG_(get_SP)(tid)) == 0) {
    on_altstack = True;
    esp_top_of_frame = (Addr)(tst->altstack.ss_sp) + tst->altstack.ss_size;
    if (VG_(clo_trace_signals))
      VG_(dmsg)
    ("delivering signal %d (%s) to thread %u: "
  ...
  /* This may fail if the client stack is busted; if that happens,
     ...
     signal handler. */
  VG_(sigframe_create)
  (tid, on_altstack, esp_top_of_frame, siginfo, uc,
   scss.scss_per_sig[sigNo].scss_handler, scss.scss_per_sig[sigNo].scss_flags,
   &tst->sig_mask, scss.scss_per_sig[sigNo].scss_restorer);
}
...
static void default_action(const vki_siginfo_t *info, ThreadId tid) {
  ...
  ("delivering %d (code %d) to default handler; action: %s%s\n", sigNo,
   info->si_code, terminate ? "terminate" : "ignore", core ? "+core" : "");

  if (!terminate)
    return; /* nothing to do */

#if defined(VGO_linux)
  if (terminate && (tst->ptrace & VKI_PT_PTRACED) && (sigNo != VKI_SIGKILL)) {
    ...
    if (is_signal_from_kernel(tid, sigNo, info->si_code)) {
      ...
      case VKI_SIGSEGV:
        ...
        case VKI_SEGV_ACCERR:
          event = "Bad permissions for mapped region";
          break;
      ...
      case VKI_SIGILL:
        switch (info->si_code) {
        case VKI_ILL_ILLOPC:
          event = "Illegal opcode";
          break;
        case VKI_ILL_ILLOPN:
          event = "Illegal operand";
          break;
        case VKI_ILL_ILLADR:
          event = "Illegal addressing mode";
          break;
        case VKI_ILL_ILLTRP:
          event = "Illegal trap";
          break;
        case VKI_ILL_PRVOPC:
          event = "Privileged opcode";
          break;
        case VKI_ILL_PRVREG:
          event = "Privileged register";
          break;
        case VKI_ILL_COPROC:
          event = "Coprocessor error";
          break;
        case VKI_ILL_BADSTK:
          event = "Internal stack error";
          break;
        }
        break;
      ...
      case VKI_SIGFPE:
        switch (info->si_code) {
        case VKI_FPE_INTDIV:
          event = "Integer divide by zero";
          break;
        case VKI_FPE_INTOVF:
          event = "Integer overflow";
          break;
        case VKI_FPE_FLTDIV:
          event = "FP divide by zero";
          break;
        case VKI_FPE_FLTOVF:
          event = "FP overflow";
          break;
        ...
        case VKI_FPE_FLTRES:
          ...
          break;
        case VKI_FPE_FLTINV:
          event = "FP invalid operation";
          break;
        case VKI_FPE_FLTSUB:
          event = "FP subscript out of range";
          break;

          /* Solaris-specific codes. */
#if defined(VKI_FPE_FLTDEN)
        case VKI_FPE_FLTDEN:
          event = "FP denormalize";
          break;
#endif
        }
        break;
      ...
      case VKI_SIGBUS:
        switch (info->si_code) {
        case VKI_BUS_ADRALN:
          event = "Invalid address alignment";
          break;
        case VKI_BUS_ADRERR:
          event = "Non-existent physical address";
          break;
        case VKI_BUS_OBJERR:
...
#endif
      ExeContext *ec =
          VG_(am_is_valid_for_client)(VG_(get_SP)(tid), sizeof(Addr),
                                      VKI_PROT_READ)
              ? VG_(record_ExeContext)(tid, first_ip_delta)
              : VG_(record_depth_1_ExeContext)(tid, first_ip_delta);
      vg_assert(ec);
...
static void deliver_signal(ThreadId tid, const vki_siginfo_t *info,
  ...
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("delivering signal %d (%s):%d to thread %u\n", sigNo, VG_(signame)(sigNo),
   info->si_code, tid);
...
/* Make a signal pending for a thread, for later delivery.
   ...
   specific thread.
*/
static void queue_signal(ThreadId tid, const vki_siginfo_t *si) {
  ThreadState *tst;
  SigQueue *sq;
  ...
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("Queueing signal %d (idx %d) to thread %u\n", si->si_signo, sq->next, tid);
  ...
  */
  if (sq->sigs[sq->next].si_signo != 0)
    VG_(umsg)
  ("Signal %d being dropped from thread %u's queue\n",
...
static vki_siginfo_t *next_queued(ThreadId tid, const vki_sigset_t *set) {
  ...
  do {
    ...
    if (sq->sigs[idx].si_signo != 0 &&
      ...
      if (VG_(clo_trace_signals))
        VG_(dmsg)
      ("Returning queued signal %d (idx %d) for thread %u\n",
...
#elif defined(VGP_amd64_solaris)
  __asm__ __volatile__("xorq %%rbp, %%rbp\n"
                       "movq %[sp], %%rsp\n"
                       "jmp async_signalhandler\n"
                       :
...
static void async_signalhandler(Int sigNo, vki_siginfo_t *info,
  ...
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("async signal handler: signal=%d, tid=%u, si_code=%d, "
   "exitreason %s\n",
   sigNo, tid, info->si_code, VG_(name_of_VgSchedReturnCode)(tst->exitreason));
  ...
  /* (1) */
  VG_(fixup_guest_state_after_syscall_interrupted)
  (tid, VG_UCONTEXT_INSTR_PTR(uc), sres,
   !!(scss.scss_per_sig[sigNo].scss_flags & VKI_SA_RESTART), uc);
  ...
  if (tst->exitreason != VgSrc_FatalSig && !is_sig_ign(info, tid))
    deliver_signal(tid, info, uc);
...
Bool VG_(extend_stack)(ThreadId tid, Addr addr) {
  ...
  if (!VG_(am_extend_into_adjacent_reservation_client)(
    ...
    if (overflow)
      VG_(umsg)
    ("Stack overflow in thread #%u: can't grow stack to %#lx\n", tid,
     new_stack_base);
    else VG_(umsg)("Cannot map memory to grow the stack for thread #%u "
                   "to %#lx\n",
                   tid, new_stack_base);
    return False;
  }
...
void VG_(set_call_fault_catcher_in_generated)(Bool should_call_in_generated) {
  call_fault_catcher_in_generated_code = should_call_in_generated;
}

static void sync_signalhandler_from_user(ThreadId tid, Int sigNo,
                                         vki_siginfo_t *info,
                                         struct vki_ucontext *uc) {
  ThreadId qtid;
  ...
  if (VG_(threads)[tid].status == VgTs_WaitSys
    ...
    if (info->VKI_SIGINFO_si_pid == 0) {
      ...
      ("Signal %d (%s) appears to have lost its siginfo; "
       ...
       sigNo, VG_(signame)(sigNo));
      VG_(printf)
      ("  This may be because one of your programs has consumed your ration "
       "of\n"
       "  siginfo structures.  For more information, see:\n"
       "    http://kerneltrap.org/mailarchive/1/message/25599/thread\n"
       "  Basically, some program on your system is building up a large queue "
       "of\n"
       "  pending signals, and this causes the siginfo data for other signals "
       "to\n"
       "  be dropped because it's exceeding a system limit.  However, "
       "Valgrind\n"
       "  absolutely needs siginfo for SIGSEGV.  A workaround is to track down "
       "the\n"
       "  offending program and avoid running it while using Valgrind, but "
       "there\n"
       "  is no easy way to do this.  Apparently the problem was fixed in "
       "kernel\n"
       "  2.6.12.\n");
...
static Bool extend_stack_if_appropriate(ThreadId tid, vki_siginfo_t *info) {
  ...
  seg_next = seg ? VG_(am_next_nsegment)(seg, True /*fwds*/) : NULL;

  if (VG_(clo_trace_signals)) {
    if (seg == NULL)
      VG_(dmsg)
    ("SIGSEGV: si_code=%d faultaddr=%#lx tid=%u ESP=%#lx "
     "seg=NULL\n",
     info->si_code, fault, tid, esp);
    else VG_(dmsg)("SIGSEGV: si_code=%d faultaddr=%#lx tid=%u ESP=%#lx "
                   "seg=%#lx-%#lx\n",
                   info->si_code, fault, tid, esp, seg->start, seg->end);
  }

  if (info->si_code == VKI_SEGV_MAPERR && seg && seg->kind == SkResvn &&
      seg->smode == SmUpper && seg_next && seg_next->kind == SkAnonC &&
...
static void sync_signalhandler_from_kernel(ThreadId tid, Int sigNo,
                                           vki_siginfo_t *info,
                                           struct vki_ucontext *uc) {
  ...
  if (fault_catcher) {
    vg_assert(call_fault_catcher_in_generated_code ||
  ...
  if (extend_stack_if_appropriate(tid, info)) {
    ...
    VG_(dmsg)
    ("si_code=%d;  Faulting address: %p;  sp: %#lx\n", info->si_code,
     info->VKI_SIGINFO_si_addr, (Addr)VG_UCONTEXT_STACK_PTR(uc));
...
static void sync_signalhandler(Int sigNo, vki_siginfo_t *info,
  ...
  vg_assert(sigNo == VKI_SIGSEGV || sigNo == VKI_SIGBUS ||
            sigNo == VKI_SIGFPE || sigNo == VKI_SIGILL || sigNo == VKI_SIGTRAP);
  ...
  if (VG_(clo_trace_signals)) {
    ...
    ("sync signal handler: "
     ...
     sigNo, info->si_code, VG_(get_IP)(tid), (Addr)VG_UCONTEXT_INSTR_PTR(uc),
...
static __attribute((unused)) void pp_ksigaction(vki_sigaction_toK_t *sa) {
  Int i;
  VG_(printf)
...
void VG_(poll_signals)(ThreadId tid) {
  ...
  if (tst->exitreason != VgSrc_None) {
    ...
    if (VG_(clo_trace_signals))
      VG_(dmsg)
    ("poll_signals: not polling as thread %u is exitreason %s\n", tid,
  ...
  if (sip == NULL && VG_(sigtimedwait_zero)(&pollset, &si) > 0) {
    if (VG_(clo_trace_signals))
      VG_(dmsg)
    ("poll_signals: got signal %d for thread %u exitreason %s\n", si.si_signo,
  ...
  if (sip != NULL) {
    ...
    if (VG_(clo_trace_signals))
      VG_(dmsg)
    ("Polling found signal %d for tid %u exitreason %s\n", sip->si_signo, tid,
     VG_(name_of_VgSchedReturnCode)(tst->exitreason));
...
void VG_(sigstartup_actions)(void) {
  ...
  */
  ...
  if (VG_(clo_trace_signals))
    VG_(dmsg)
  ("Max kernel-supported signal is %d, VG_SIGVGKILL is %d\n", VG_(max_signal),
   VG_SIGVGKILL);
