...
static
void SE_(thread_exit)(ThreadId tid) {
...
static
void SE_(start_client_code)(ThreadId tid, ULong blocks_dispatched)
{
    if(!client_running && tid == target_id) {
...
static
void SE_(stop_client_code)(ThreadId tid, ULong blocks_dispatched)
{
//    VG_(umsg)("Thread %u stopped executing at instruction 0x%lx with blocks_dispatched=%llu\n", tid, VG_(get_IP)(tid), blocks_dispatched);
}

static
IRSB* SE_(instrument) ( VgCallbackClosure* closure,
                      IRSB* bb,
...
//    if(target_function_running) {
        ...
        const HChar *fnname = "@UNKNOWN@";
        VG_(get_fnname)(de, closure->nraddr, &fnname);
        VG_(umsg)("Thread %u requested translation of block starting at 0x%lx in function %s\n", closure->tid, closure->nraddr, fnname);
//    }
    return bb;
}
...
static void SE_(pre_clo_init)(void)
{
   ...
   VG_(basic_tool_funcs)        (SE_(post_clo_init),
                                 SE_(instrument),
                                 SE_(fini));

    VG_(needs_command_line_options)(SE_(process_cmd_line_option),
                                    SE_(print_usage),
                                    SE_(print_debug_usage));
